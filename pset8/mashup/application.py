...
DEBUG = True
SECRET_KEY = 'super secret squirrel key'
...
def tester():
    ...
    place = db.execute("SELECT * FROM places WHERE postal_code=49788")
    place = place[0]
    lat = place["latitude"]
    lon = place["longitude"]
    return jsonify(db.execute("SELECT * FROM places WHERE latitude=:lat and longitude=:lon", lat=lat, lon=lon))
